...
public class Main {
    ...
    // Scanner global para leer desde la consola
    ...
    private static void nuevoPedido() {
        ...
        try {
            ...
            // LISTA TEMPORAL EN MEMORIA (Requisito 5.4.1)
                ...
                try {
                    ...
                    if (optProd.isPresent() && !optProd.get().isEliminado()) {
                        ...
                        if (!prod.getDisponible()) {
                            ...
                            // Calculamos el stock considerando lo que ya pusimos en el carrito temporal
            ...
            // INICIO DEL GUARDADO (Una vez confirmado el carrito)
            ...
            try {
                // Delegamos la transaccion atómica al repositorio
                pedidoRepo.guardarPedidoConDetalles(usuario, pedido, carritoTemporal);

                System.out.println("¡Pedido registrado exitosamente!");

                // Calculamos el total para mostrarlo por pantalla
    ...
    private static void listarPedidosUsuario() {
        ...
        // Correccion 3: Listar usuarios antes de pedir el ID
        listarUsuarios();
    ...
    private static void reportePedidosPorEstado() {
        System.out.println("\n- PEDIDOS POR ESTADO -");
        // Asegurate de que estos textos coincidan con los de tu archivo Enum Estado
        System.out.println("Estados posibles: PENDIENTE, CONFIRMADO, TERMINADO, CANCELADO");
        System.out.print("Ingrese el estado a buscar: ");
        String estadoStr = scanner.nextLine().trim().toUpperCase();

        try {
            // Convierte el texto ingresado al Enum correspondiente
    ...
    private static void reporteTotalFacturado() {
        ...
        // El formato exacto que pide el PDF para evitar errores de coma flotante
