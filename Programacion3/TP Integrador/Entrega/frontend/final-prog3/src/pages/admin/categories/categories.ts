...
async function initCategorias() {
    ...
    // Cargar JSON
    try {
        const guardadas = localStorage.getItem("categorias-locales");
        if (guardadas) {
            categoriasGlobal = JSON.parse(guardadas);
        } else {
            const response = await fetch('/data/categorias.json');
            categoriasGlobal = await response.json();
            guardarCategoriasEnMemoria();
        }
        renderTabla();
    } catch (error) {
...
function guardarCategoriasEnMemoria() {
    localStorage.setItem("categorias-locales", JSON.stringify(categoriasGlobal));
}
