...
async function initPedidos() {
    ...
    try {
        ...
        pedidosGlobal = await pedRes.json();
...
function renderPedidos(filtroEstado: string = "TODOS") {
    const contenedor = document.getElementById("contenedor-pedidos")!;
    contenedor.innerHTML = '';

    let pedidosFiltrados = pedidosGlobal;
    if (filtroEstado !== "TODOS") {
        pedidosFiltrados = pedidosGlobal.filter(p => p.estado === filtroEstado);
    }
    ...
    pedidosFiltrados.forEach(pedido => {
        // Extraer el nombre del cliente a prueba de fallos (JSON anidado o memoria plana)
        ...
        contenedor.innerHTML += `
            <div class="pedido-card">
                <div class="pedido-info-basica">
                    ...
                    <p>📦 ${cantidadItems} productos en este pedido</p>
                </div>
...
function configurarEventos() {
    ...
    formEstado.addEventListener("submit", (e) => {
        ...
        if (pedido) {
            ...
            alert(`El estado del pedido #${idPedido} ha sido actualizado a ${nuevoEstado} en memoria.`);
        }
...
function abrirModalPedido(id: number) {
    ...
    // Si el JSON trae estados raros, forzamos a PENDIENTE visualmente para que no se rompa el select
    ...
    if (pedido.detalles) {
        pedido.detalles.forEach((det: any) => {
            // Leer el nombre del producto de forma dinámica
