...
async function initProductos() {
    ...
    try {
        // Cargar Productos y Categorías a la vez
...
function llenarSelectCategorias() {
    ...
    select.innerHTML = '';
    // Solo mostrar categorías que no estén eliminadas
...
function renderTabla() {
    ...
    activos.forEach(prod => {
        // Buscamos el ID dependiendo de si viene del JSON (prod.categoria.id) 
        // o si fue creado en memoria desde el formulario (prod.categoriaId)
...
function abrirModalEdicion(id: number) {
    ...
    // Extraemos el ID de la categoría a prueba de fallos (JSON vs Memoria)
    const idCatProducto = prod.categoria?.id || prod.categoriaId || '';
    ...
    // Le pasamos el ID extraído de forma segura
